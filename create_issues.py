...
issues_to_create = [
    {"group": "dataproc", "kind": "DataprocCluster", "shortkind": "Cluster", "plural": "dataprocclusters"},
    {"group": "osconfig", "kind": "OSConfigOSPolicyAssignment", "shortkind": "OSPolicyAssignment", "plural": "osconfigospolicyassignments"},
    {"group": "networkconnectivity", "kind": "NetworkConnectivitySpoke", "shortkind": "Spoke", "plural": "networkconnectivityspokes"}
]

template = """As part of moving resources from terraform controllers to direct controllers (Epic #5954), we need to create the Go types for `{kind}`.
...
3.  **Generate Scaffolding**:
    Run `apis/{group}/v1beta1/generate.sh`. This should create `apis/{group}/v1beta1/{shortkind_lower}_types.go`.

4.  **Iterate on Types**:
    Compare the generated CRD with the existing one using `dev/tasks/diff-crds`.
    Modify `apis/{group}/v1beta1/{shortkind_lower}_types.go` until the CRD matches the existing one at `config/crds/resources/apiextensions.k8s.io_v1_customresourcedefinition_{plural}.{group}.cnrm.cloud.google.com.yaml`.
...
8. **Generate Mappers**:
   - Running `dev/tasks/generate-types-and-mappers` will generate the mapper code once the `apis/{group}/v1beta1/{shortkind_lower}_types.go` file is generating an equivalent CRD.
...
for item in issues_to_create:
    title = f"Create generate.sh and types.go files for {item['group'].capitalize()} {item['kind']}"
    body = template.format(
        kind=item['kind'],
        group=item['group'],
        shortkind=item['shortkind'],
        shortkind_lower=item['shortkind'].lower(),
        plural=item['plural']
    )
    
    with open(f"issue_{item['group']}.md", "w") as f:
        f.write(body)
    
    cmd = [
        "gh", "issue", "create",
        "--title", title,
        "--body-file", f"issue_{item['group']}.md"
    ]
